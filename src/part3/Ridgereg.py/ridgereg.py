...
ridge_cv_pb = RidgeCV(alphas=np.logspace(-3, 3, 100), cv=5)
ridge_cv_pb.fit(X_train, y_train)

print("Best alpha value for PerformanceBased salary:", ridge_cv_pb.alpha_)

#Predictions and metrics
y_pred_performance = ridge_cv_pb.predict(X_test)
...
print("Model Evaluation Metrics for PerformanceBased Salary:")
...
#Ridge with cross-validation
ridge_cv_fg = RidgeCV(alphas=np.logspace(-3, 3, 100), cv=5)
ridge_cv_fg.fit(X_train, y_train)

print("\nBest alpha value for FixedGrowth salary:", ridge_cv_fg.alpha_)

#Predictions and metrics
y_pred_fixedgrowth = ridge_cv_fg.predict(X_test)
...
print("Model Evaluation Metrics for FixedGrowth Salary:")
...
df.to_csv("predicted_salaries_part3_ridge.csv", index=False)
...
plt.title(f"Ridge: Actual vs Predicted (PerformanceBased) R² = {r2_performance:.4f}")
plt.grid(True)
plt.tight_layout()
plt.show()
plt.close()
...
plt.xlabel("Actual")
plt.ylabel("Predicted")
plt.title(f"Ridge: Actual vs Predicted (FixedGrowth) R² = {r2_fixedgrowth:.4f}")
plt.grid(True)
plt.tight_layout()
plt.show()
plt.close()
...
plt.title("Ridge Coefficients - PerformanceBased Salary")
...
plt.title("Ridge Coefficients - FixedGrowth Salary")
plt.xlabel("Coefficient Value")
plt.grid(True)
plt.tight_layout()
plt.show()