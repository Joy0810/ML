...
# Best model indices (for reference only, not for color highlighting now)
best_r2_index = np.argmax(r2_scores)
best_rmse_index = np.argmin(rmse)
best_mape_index = np.argmin(mape)

# Horizontal bar plot function (uniform color, adjusted layout)
def plot_metric(values, title, xlabel, filename, color='skyblue'):
    ...
    plt.tight_layout(pad=1.5)  # Ensure nothing is cut off
